...
int construct_module_range_table()
{
  ...
  sceClibMemset(g_modlist, 0, sizeof(g_modlist));

  sceClibMemset(g_segList, -1, sizeof(g_segList));
  
  int ret = sceKernelGetModuleList(0xFF, g_modlist, &count);
  if(ret < 0)
    return ret;

  open_global_log();
  sceClibSnprintf(sprintfBuffer, 256, "number of modules: %d\n", count);
  ...
  for (int m = 0; m < count; m++) 
  {
    ...
    ret = sceKernelGetModuleInfo(g_modlist[m], &minfo);
    if(ret < 0)
      return ret;
    ...
    for (int s = 0; s < 4; ++s) 
    {
      ...
      sceClibMemset(g_segList[segInfoIndex].moduleName, 0, 30);
      sceClibMemcpy(g_segList[segInfoIndex].moduleName, minfo.module_name, 28);
